...
# cv2.waitKey(0)  # 等待按鍵，按下任意鍵關閉
# cv2.destroyAllWindows()  # 關閉視窗

gray_img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # cvt -> convert（轉換）
# cv2.imshow('Usagi_gray', gray_img)  # 用 OpenCV 顯示, 放視窗標題跟圖片物件
# cv2.waitKey(0)  # 等待按鍵，按下任意鍵關閉
# cv2.destroyAllWindows()  # 關閉視窗
...
# 如果享用 Matplotlib 看彩圖，需要先轉換成 RGB 格式
...
# 顯示原始圖片
plt.subplot(1, 5, 1)  # 圖片排列方式及位置: 1列5張圖位置第1張 -> (1,5,1)
"""
+-------+-------+-------+-------+-------+
